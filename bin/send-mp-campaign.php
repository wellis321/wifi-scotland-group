 ...
 *
 * Sends one personalised email per row in data/mp-roster.csv (57 Scottish MPs, all
 * sourced directly from the UK Parliament Members API — no third-party aggregators),
 * via the Resend batch API. Every send is logged to mp_campaign_sends so re-running
 * this script is safe — already-sent rows are skipped, only rows that previously
 * failed are retried.
 *
 ...
 *
 * Usage:
 *   php bin/send-mp-campaign.php --dry-run
 *     Renders the first few emails to STDOUT. Sends nothing, touches no DB rows.
 *
 *   php bin/send-mp-campaign.php --test-to=you@example.com --test-count=5
 *     Sends 5 real merge-personalised emails (using 5 real roster rows) but all
 *     addressed to --test-to instead of the real MP. Subject is prefixed "[TEST]"
 *     so it's unmistakable in an inbox. Never touches the DB.
 *
 *   php bin/send-mp-campaign.php --limit=20
 *     A real send, capped at the first 20 not-yet-sent MPs. Asks for interactive
 *     confirmation first unless --yes is also passed.
 *
 *   php bin/send-mp-campaign.php
 *     A real send to every not-yet-sent MP (57 total). Asks for interactive
 *     confirmation first unless --yes is also passed.
 *
 * Flags:
...
const DEFAULT_CAMPAIGN_SLUG = 'mp-accountability-2026-09';
const ROSTER_CSV_PATH       = __DIR__ . '/../data/mp-roster.csv';
...
function already_sent_emails(string $campaignSlug): array
{
    $stmt = campaign_db()->prepare(
        'SELECT email FROM mp_campaign_sends WHERE campaign_slug = ? AND status = ?'
    );
...
{
    $stmt = campaign_db()->prepare(
        'INSERT INTO mp_campaign_sends
            (campaign_slug, full_name, party, constituency, email, status, resend_id, error_message, subject, body_text)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    $stmt->execute([$campaignSlug, $row['name'], $row['party'], $row['constituency'], $row['email'], $status, $resendId, $error, $subject, $bodyText]);
}
...
if ($dryRun) {
    ...
    foreach ($sample as $row) {
        ...
        fwrite(STDOUT, "TO: {$row['name']} <{$row['email']}> ({$row['constituency']})\n");
...
foreach (array_chunk($recipients, BATCH_SIZE) as $batch) {
    ...
    foreach ($batch as $i => $row) {
        if ($result['ok']) {
            ...
            $sentCount++;
            fwrite(STDOUT, "sent  {$row['name']} <{$row['email']}> ({$row['constituency']})\n");
        } else {
            if (!$isTestMode) {
